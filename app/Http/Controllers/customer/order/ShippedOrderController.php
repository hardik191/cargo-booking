...
class ShippedOrderController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($user->hasRole('Customer')) {
            $data['title'] =  'Shipped Order List' . ' || ' . get_system_name();
            $data['header'] = array(
                'title' => 'Shipped Order List',
                'breadcrumb' => array(
                    'Home' => route('dashboard1'),
                    'Create Order' => route('create-order'),
                    'Shipped Order List' => 'Shipped Order List',
                )
            );
            return view('customer.pages.order.shipped_order.list', $data);
        } else {
            if ($user->can('shipped-order list')) {
                $data['title'] =  'Shipped Order List' . ' || ' . get_system_name();
                $data['header'] = array(
                    'title' => 'Shipped Order List',
                    'breadcrumb' => array(
    ...
    public function show(string $id)
    {
        ...
        if ($user->hasRole('Customer')) {
            ...
            $data['title'] =  'Shipped Order View' . ' || ' . get_system_name();
            $data['header'] = array(
                'title' => 'Shipped Order View',
                'breadcrumb' => array(
                    'Home' => route('dashboard1'),
                    'Shipped Order' => route('shipped-order1'),
                    'Shipped Order View' => 'Shipped Order View',
                )
            );
        } else {
            ...
            $data['header'] = array(
                ...
                'breadcrumb' => array(
                    'Home' => route('dashboard'),
                    'Shipped Order' => route('shipped-order'),
                    'Shipped Order View' => 'Shipped Order View',
                )
            );
        }
        ...
        if (is_null($data['order_details'])) {
            if ($user->hasRole('Customer')) {
                return redirect()->route('shipped-order1')->with('warning', 'Order not found.');
            } else {
                return redirect()->route('shipped-order')->with('warning', 'Order not found.');
            }
        }
    ...
    public function ajaxcall(Request $request)
    {
        ...
        switch ($action) {
            case 'getdatatable':
                ...
                if ($user->hasRole('Customer')) {
                    $query = Order::where('is_deleted', '1')->where('order_status', '4')->where('add_by', $user->id);
                } else {
                    $query = Order::where('is_deleted', '1')->where('order_status', '4');
                }
                ...
                foreach ($resultArr as $row) {
                    ...
                    if ($user->hasRole('Customer')) {
                        $actionhtml .= '<li><a class="dropdown-item" href="' . route('view-shipped-order1', $row['id']) . '" ><i class="fa fa-eye text-info"></i> View</a></li>';
                    } else {
                        if ($user->can('shipped-order view')) {
                    ...
                    if ($user->hasRole('Customer')) {
                        $orderCodeBadge = ' <a href="' . route('view-shipped-order1', $row['id']) . '" class="">' . $orderCodeBadge . '</a>';
                    } else {
                        if ($user->can('shipped-order view')) {
                            $orderCodeBadge = ' <a href="' . route('view-shipped-order', $row['id']) . '" class="">' . $orderCodeBadge . '</a>';
                        } else {
