...
class RejectedOrderController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        if ($user->hasRole('Customer')) {
            $data['title'] =  'Rejected Order List' . ' || ' . get_system_name();
            $data['header'] = array(
                'title' => 'Rejected Order List',
                'breadcrumb' => array(
                    'Home' => route('dashboard1'),
                    'Create Order' => route('create-order'),
                    'Rejected Order List' => 'Rejected Order List',
                )
            );
            return view('customer.pages.order.rejected_order.list', $data);
        } else {
            if ($user->can('rejected-order list')) {
                $data['title'] =  'Rejected Order List' . ' || ' . get_system_name();
                $data['header'] = array(
                    'title' => 'Rejected Order List',
                    'breadcrumb' => array(
    ...
    public function show(string $id)
    {
        ...
        if ($user->hasRole('Customer')) {
            ...
            $data['title'] =  'Rejected Order View' . ' || ' . get_system_name();
            $data['header'] = array(
                'title' => 'Rejected Order View',
                'breadcrumb' => array(
                    'Home' => route('dashboard1'),
                    'Rejected Order' => route('rejected-order1'),
                    'Rejected Order View' => 'Rejected Order View',
                )
            );
        } else {
            ...
            $data['header'] = array(
                ...
                'breadcrumb' => array(
                    'Home' => route('dashboard'),
                    'Rejected Order' => route('rejected-order'),
                    'Rejected Order View' => 'Rejected Order View',
                )
            );
        }
        ...
        if (is_null($data['order_details'])) {
            if ($user->hasRole('Customer')) {
                return redirect()->route('rejected-order1')->with('warning', 'Order not found.');
            } else {
                return redirect()->route('rejected-order')->with('warning', 'Order not found.');
            }
        }
    ...
    public function ajaxcall(Request $request)
    {
        ...
        switch ($action) {
            case 'getdatatable':
                ...
                if ($user->hasRole('Customer')) {
                    $query = Order::where('is_deleted', '1')->where('order_status', '3')->where('add_by', $user->id);
                } else {
                    $query = Order::where('is_deleted', '1')->where('order_status', '3');
                }
                ...
                foreach ($resultArr as $row) {
                    ...
                    if ($user->hasRole('Customer')) {
                        $actionhtml .= '<li><a class="dropdown-item" href="' . route('view-rejected-order1', $row['id']) . '" ><i class="fa fa-eye text-info"></i> View</a></li>';
                    } else {
                        if ($user->can('rejected-order view')) {
                            $actionhtml .= '<li><a class="dropdown-item" href="' . route('view-rejected-order', $row['id']) . '" ><i class="fa fa-eye text-info"></i> View</a></li>';
                        }
                    }
                    ...
                    //     5 => '<span class="badge badge-outline badge-success"></span>',
                    //     default => '<span class="badge badge-outline badge-secondary">Unknown</span>',
                    // };

                    if ($user->hasRole('Customer')) {
                        $orderCodeBadge = ' <a href="' . route('view-rejected-order1', $row['id']) . '" class="">' . $orderCodeBadge . '</a>';
                    } else {
                        if ($user->can('rejected-order view')) {
                            $orderCodeBadge = ' <a href="' . route('view-rejected-order', $row['id']) . '" class="">' . $orderCodeBadge . '</a>';
                        } else {
