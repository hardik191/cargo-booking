...
class DeliveryOrderController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($user->hasRole('Customer')) {
            $data['title'] =  'Delivery Order List' . ' || ' . get_system_name();
            $data['header'] = array(
            ...
            return view('customer.pages.order.delivery_order.list', $data);
        } else {
            if ($user->can('delivery-order list')) {
                $data['title'] =  'Delivery Order List' . ' || ' . get_system_name();
                $data['header'] = array(
                    'title' => 'Delivery Order List',
                    'breadcrumb' => array(
    ...
    public function show($id)
    {
        ...
        if ($user->hasRole('Customer')) {
            ...
            $data['title'] =  'Delivery Order View' . ' || ' . get_system_name();
            $data['header'] = array(
                'title' => 'Delivery Order View',
                'breadcrumb' => array(
                    'Home' => route('dashboard1'),
                    'Delivery Order' => route('delivery-order1'),
                    'Delivery Order View' => 'Delivery Order View',
                )
            );
        } else {
            ...
            $data['header'] = array(
                ...
                'breadcrumb' => array(
                    'Home' => route('dashboard'),
                    'Delivery Order' => route('delivery-order'),
                    'Delivery Order View' => 'Delivery Order View',
                )
            );
        }
        ...
        if (is_null($data['order_details'])) {
            if ($user->hasRole('Customer')) {
                return redirect()->route('delivery-order1')->with('warning', 'Order not found.');
            } else {
                return redirect()->route('delivery-order')->with('warning', 'Order not found.');
            }
        }
        ...
        if ($user->hasRole('Customer')) {
            ...
            if ($user->can('delivery-order view')) {
                return view('customer.pages.order.accepted_order.view', $data);
            } else {
                return redirect()->route('access-denied');
            }
    ...
    public function ajaxcall(Request $request)
    {
        ...
        switch ($action) {
            case 'getdatatable':
                ...
                if ($user->hasRole('Customer')) {
                    $query = Order::where('is_deleted', '1')->where('order_status', '5')->where('add_by', $user->id);
                } else {
                    $query = Order::where('is_deleted', '1')->where('order_status', '5');
                }
                ...
                foreach ($resultArr as $row) {
                    ...
                    if ($user->hasRole('Customer')) {
                        $actionhtml .= '<li><a class="dropdown-item" href="' . route('view-delivery-order1', $row['id']) . '" ><i class="fa fa-eye text-info"></i> View</a></li>';
                    } else {
                        if ($user->can('delivery-order view')) {
                            $actionhtml .= '<li><a class="dropdown-item" href="' . route('view-delivery-order', $row['id']) . '" ><i class="fa fa-eye text-info"></i> View</a></li>';
                        }
                    }
                    ...
                    if ($user->hasRole('Customer')) {
                        $orderCodeBadge = ' <a href="' . route('view-delivery-order1', $row['id']) . '" class="">' . $orderCodeBadge . '</a>';
                    } else {
                        if ($user->can('delivery-order view')) {
                            $orderCodeBadge = ' <a href="' . route('view-delivery-order', $row['id']) . '" class="">' . $orderCodeBadge . '</a>';
                        } else {
